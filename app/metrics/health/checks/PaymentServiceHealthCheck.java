...
public class PaymentServiceHealthCheck extends AbstractServiceHealthCheck {
    ...
    @Inject
    public PaymentServiceHealthCheck(WSClient client,
                                     @ConfigurationValue(key = "paymentService.adminURL") String serviceAdminURL,
                                     @ConfigurationValue(key = "metrics.healthcheck.timeout") int timeout) {
        super(client);

        checkArgument(!isNullOrEmpty(serviceAdminURL), "Payment service admin URL is required");
        ...
        try {
            ...
            throw new IllegalArgumentException("Payment service admin URL is invalid", ex);
        }
